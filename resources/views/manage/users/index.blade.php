                        ...
                        <tr>
                            ...
                            <td class="td-center">
                                <div class="hstack justify-content-evenly d-flex gap-1">
                                    ...
                                    @can('restore')
                                        @if ($user->deleted_at)
                                            <form method="POST" action="{{ route('manage.users.restore', $user->id) }}">
                                                @csrf
                                                @method('PUT')
                                                <x-button tooltip="คืนค่า" size="sm" icon="restart_alt" icon-size="20"
                                                    color="success" :href="route('manage.users.destroy', $user->id)" />
                                            </form>
                                        @endif
                                    @endcan
                                    <!-- Action-Button -->
